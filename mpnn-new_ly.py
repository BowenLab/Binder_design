...
class ProteinMPNN_runner():
    ...
    def relax_pose(self, sample_feats):
        ...
        print(f'{get_timestamp()} Running FastRelax')

        self.FastRelax.apply(sample_feats.pose)

        print(f"{get_timestamp()} Completed one cycle of FastRelax in {int(time.time()) - relaxT0} seconds")

    def sequence_optimize(self, sample_feats):
        mpnn_t0 = time.time()

        pid = os.getpid()
        pdbfile = f'temp_{pid}.pdb'
        ...
        try:
            ...
            print(f"Error removing PDB file: {e}")
        ...
        print(f'{get_timestamp()} ProteinMPNN generated {len(sequences)} sequences in {int(time.time() - mpnn_t0)} seconds')
    ...
    def run_model(self, tag, args):
        ...
        while retries < max_retries:
            print(f"Attempting pose: {tag} (retry {retries + 1}/{max_retries})")
            ...
            try:
                ...
                print(f"{get_timestamp()} Struct: {tag} reported success in {seconds} seconds")
                return True  # 返回True表示成功

            except Exception as e:
                retries += 1
                seconds = int(time.time() - t0)
                print(f"{get_timestamp()} Struct with tag {tag} failed in {seconds} seconds with error: {str(e)}")
                if retries >= max_retries:
                    print(f"{get_timestamp()} Struct with tag {tag} failed after {max_retries} retries.")
...
for pdb in struct_manager.iterate():
    ...
    if args.debug:
        success = proteinmpnn_runner.run_model(pdb, args)
    else:
        t0 = time.time()
        try:
            success = proteinmpnn_runner.run_model(pdb, args)
            ...
            seconds = int(time.time() - t0)
            print(f"Struct with tag {pdb} failed in {seconds} seconds with error: {e}")
